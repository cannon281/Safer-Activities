...
def pose_points_yolo5(detector,image,pose,tracker,tensorrt,det_type="yolov8"):
    timer_det.tic()
    ...
    if det_type == "yolov8":
        detections = detector(image, verbose=False, conf=0.2, iou=0.7)
    ...
    dets = dets[dets[:,5] == 0.] # for filtering only person classes
...
def pose_points_yolo5_with_heatmaps(detector,image,pose,tracker,tensorrt):
    timer_det.tic()
    ...
    # Interpret the <models.common.Detections> object
    # print(dir(detections))
    # print(detections.s) # input image shape
    ...
    if len(res) > 1:
        return res,online_tlwhs,online_ids,online_scores, out.cpu().numpy()#,pts2
    else:
        try:
            out = out.cpu().numpy()
        except:
            out = np.empty((17,64,48), float)
        return res[0],online_tlwhs,online_ids,online_scores, out #,pts2

...
def pose_points_yolo5_deepsort(detector,image,pose,tracker,tensorrt,det_type="yolov8"):
    timer_det.tic()
    ...
    # For yolo-v8:
    if det_type == "yolov8":
        dets = detections[0].boxes.xyxy # For Yolo-v8
        confs=detections[0].boxes.conf.unsqueeze(1)
        dets = torch.cat((dets, confs), dim=1)
        classes = detections[0].boxes.cls.unsqueeze(1)
        dets = torch.cat((dets, classes), dim=1)
    else:
    ...
    for det in _detections:
        ...
        t = y1
        confidence = det[4]
        converted_detections.append(([l, t, w, h], confidence, 1))  # 1 is a placeholder for class

    tracks = tracker.update_tracks(converted_detections, frame=image) # bbs expected to be a list of detections, each in tuples of ( [left,top,w,h], confidence, detection_class )
    
    online_tlwhs = []
    online_ids = []
    online_scores = []
    for t in tracks:
        tlwh = t.original_ltwh  # Assuming original_ltwh is in the format [x, y, w, h]
        tid = int(t.track_id)
        # vertical = tlwh[2] / tlwh[3] > args.aspect_ratio_threshs
        if tlwh is not None and tlwh[2] * tlwh[3] > 10 :#and not vertical:
            online_tlwhs.append(tlwh)
            online_ids.append(tid)
            online_scores.append(t.det_conf)

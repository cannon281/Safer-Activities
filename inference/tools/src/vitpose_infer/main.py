...
class VitInference:
    ...
    def __init__(self,pose_path, tensorrt=False, tracking_method="bytetrack", det_type="yolov8", det_path=None):
    ...
    def inference(self,img,frame_id=0, return_heatmaps=False, return_kpt_image=False):
        frame_orig = img.copy()
        self.timer.tic()
        ...
        # keypoints_threshold = 0.3 # 0.3 seems to be working best
        ...
        if return_kpt_image:
            kpt_im = np.zeros((1080,1920,3),np.uint8)
            kpt_im[:,:,:]=255
        ...
        if len(online_ids)>0:
            ...
            # To display the bounding box and track id
            # if return_kpt_image:
            #     kpt_im = plot_tracking(
            #         kpt_im, online_tlwhs, online_ids, frame_id=frame_id, fps=1/self.timer.average_time
            #     )
            ...
            if pts is not None:
                for i, (pt, pid) in enumerate(zip(pts, online_ids)):
                    ...
                    online_im=draw_points_and_skeleton(online_im, pt, joints_dict()['coco']['skeleton'], person_index=pid,
                                                            points_color_palette='gist_rainbow', skeleton_color_palette='jet',points_palette_samples=10,confidence_threshold=0.01)
                    if return_kpt_image:
                        kpt_im = draw_points_and_skeleton(kpt_im, pt, joints_dict()['coco']['skeleton'], person_index=pid,
                                                            points_color_palette='gist_rainbow', skeleton_color_palette='jet',points_palette_samples=10,confidence_threshold=0.01)
        else:
        ...
        if return_kpt_image:
            return pts,online_ids,online_tlwhs,online_im,frame_orig, online_scores, kpt_im

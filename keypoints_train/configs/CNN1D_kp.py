...
data_root = 'data/aicactivity/' # Path of the dataset pickle file
dataset_type="normal" # normal or wheelchair
num_frames = 48 # Window size (w), 48 or 144 
skip_frames = False # Whether to skip frames or not 
skip_stride = 1 # Temporal stride, set to 3 if using a window size of 144 and 1 if a window size of 48
mode = "classification" # Classification or segmentation
...
dataset_cfg = dict(
    ...
    args = dict(
            pickle_file_path=data_root+dataset_type+"/aic_"+dataset_type+"_dataset.pkl", # The pickle file
...
model_cfg = dict(
    type='KeypointCNN1DSegment' if mode=="segmentation" else "KeypointCNN1D",
    args=dict(

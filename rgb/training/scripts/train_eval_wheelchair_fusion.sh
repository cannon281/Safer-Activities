#!/bin/bash
# =============================================================================
# Example: Train & evaluate a fusion model on the wheelchair dataset
#
# Model: VideoMAE + CNN1D skeleton concat fusion
# Evaluates on: wheelchair test set
#
# Usage:
#   bash scripts/train_eval_wheelchair_fusion.sh
#   bash scripts/train_eval_wheelchair_fusion.sh --eval-only
# =============================================================================
...
if [ "$EVAL_ONLY" = false ]; then
    echo "========== Training: VideoMAE + CNN1D Concat Fusion (wheelchair) =========="
    python train.py --config configs/wheelchair/wheelchair_videomae_cnn1d_fusion.py
fi
...
run_eval "wheelchair_videomae_cnn1d_fusion" \
    "runs/wheelchair_videomae_cnn1d_fusion_seq" \
    "$WHEELCHAIR_PKL" "$EVAL_WHEELCHAIR" "$WHEELCHAIR_FEAT"

echo ""
echo "Done! Reports in: $EVAL_WHEELCHAIR/wheelchair_videomae_cnn1d_fusion/report/"
